...
# One-time costs
WIFI_SETUP_FEE = 100  # Router/modem setup fee
...
# Auto-save defaults for input persistence
if "calc_rent" not in st.session_state:
    st.session_state.calc_rent = 0.0
if "calc_deposit" not in st.session_state:
    st.session_state.calc_deposit = 0.0
if "calc_application" not in st.session_state:
    st.session_state.calc_application = 0.0
if "calc_admin" not in st.session_state:
    st.session_state.calc_admin = 0.0
if "calc_house_type" not in st.session_state:
    st.session_state.calc_house_type = "studio"
if "calc_roommates" not in st.session_state:
    st.session_state.calc_roommates = 0
if "calc_transport_mode" not in st.session_state:
    st.session_state.calc_transport_mode = "School shuttle (free)"
if "calc_savings" not in st.session_state:
    st.session_state.calc_savings = 0.0
if "calc_uber_cost" not in st.session_state:
    st.session_state.calc_uber_cost = float(UBER_AVG_COST)
if "calc_bus_rides" not in st.session_state:
    st.session_state.calc_bus_rides = 2
if "calc_uber_rides" not in st.session_state:
    st.session_state.calc_uber_rides = 1
if "calc_scooter_rides" not in st.session_state:
    st.session_state.calc_scooter_rides = 2
...
def calculate_transport_cost(mode, rides_per_day=0, uber_cost=UBER_AVG_COST):
    ...
    if mode in ("School shuttle (free)", "Walking", "Bike"):
        ...
        return 4 * rides_per_day * DAYS_IN_PERIOD
    return 0
...
if transport_mode == "Public transport (bus/train)":
    ...
    # Pre-fill uber cost from Fare Estimator if available and user hasn't changed it
    ...
    st.warning(f"⚠️ Estimated 60-day cost: **\\${transport_cost:.2f}** - Consider the free shuttle!")

elif transport_mode == "Scooter":
    rides_per_day = st.number_input(
        "🛴 Average scooter rides per day ($4/ride average):",
        min_value=0, max_value=20, step=1,
        key="calc_scooter_rides",
        help="Bird, Lime, and other scooter services"
    )
    transport_cost = calculate_transport_cost(transport_mode, rides_per_day)
    st.info(f"📊 Estimated 60-day cost: **\\${transport_cost:.2f}**")
...
# =============================================================================
# RESULTS SECTION (only when user clicks "View Results Below")
# =============================================================================
    ...
    with col2:
        if st.button("🔄 Start Over", use_container_width=True):
            for key in ["submitted", "calculation_results", "show_results", "calculation_success",
                        ...
                        "calc_scooter_rides", "calc_savings"]:
